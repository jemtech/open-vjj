 ...
 * 
 */
public class CombineVectorListList extends Plugin {

	public CombineVectorListList() {
	...
	private void calculate(){
		...
		List<VectorValue> valueList = new ArrayList<VectorValue>();
		for(VectorValueList values : my1ValueList.getVectorValues()){
			//add all vectors
			valueList.addAll(values.getVectorValues());
			//add a vector from start to end
			//valueList.add(new VectorValue(values.getVectorValues().get(0).getStart(), values.getVectorValues().get(values.getVectorValues().size()-1).getEnd()));
